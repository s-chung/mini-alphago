...
#   complete redesign of main class board_game and graphics user interface
...
class board_game(object):
    ...
    def play_games(self, net1, r1, net2, r2, ng, max_moves = 0, nargout = 1):
        ...
        #   max_moves (optional): the max. number of moves per game
        ...
        if max_moves <= 0:
            np0 = nx * ny * 2   # default is 2 times the board size (2 times since stones can be caputed in some games 
                                # such as Go). Actually, can be more than 2x, but limit to 2x since it is not
                                # very likely to exceed 2x.
        else:
            np0 = max_moves

        # m: max. possible number of board states in 'd'
        m = np0 * ng
        d = np.zeros((nx, ny, 3, m))
        
        # game outcome, tie(0)/black win(1)/white win(2), for each board state for each board state in 'd'
        w = np.zeros((m))

        # winning probability for each board state in 'd'
        wp = np.zeros((m))
    ...
    def play_interactive(self, net1, r1, net2, r2):
        ...
        self.cid2 = self.fig.canvas.mpl_connect('close_event', self.figure_close)
    ...
    def figure_close(self, event):
        # will get a warning message without the following two lines in python 3.x
    ...
    def push_callback2(self, event):
        ...
        if self.human_players == 1:
            self.mouse_click([])    # so that computer plays first in computer-human match
...
class game4(board_game):
    def __init__(self, nx = 8, ny = 8, name = 'othello', theme = 'basic'):
        ...
        # Inputs
        #   nx, ny: board size (only 8x8 is supported)
        self.nx = nx
        self.ny = ny
        self.name = name
        self.theme = theme
